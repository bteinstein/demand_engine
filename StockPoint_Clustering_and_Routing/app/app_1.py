...
def create_multi_stockpoint_map(selected_spids, processed_sp_dim_df, 
    ...
    for i, spid in enumerate(selected_spids):
        ...
        # Add boundary
        if spid in sp_territories_dict:
            boundary_polygon = sp_territories_dict[spid]['polygon']
            # Convert shapely geometry to GeoJSON
            boundary_geojson = boundary_polygon.__geo_interface__
            folium.GeoJson(
                boundary_geojson,
        ...
        # Add customers if data exists
        if not sp_customers.empty:
...
def main():
    ...
    if selected_spids:
        ...
        with st.sidebar.expander("📊 Selection Summary"):
            ...
            st.metric("Total Beats", len(total_coverage))
            st.metric("Total Customers", len(total_customers))
            if not total_coverage.empty:
                st.metric("Avg Area (km²)", f"{total_coverage['area_km2'].mean():.1f}")
