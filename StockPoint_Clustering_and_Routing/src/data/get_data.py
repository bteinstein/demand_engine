...
class DataFetcher:
    ...
    def __init__(self, logger=None, input_dir="./input", sql_dir="_sql"):
        ...
        self.base_data_input_filename_path = {
    ...
    def fetch_all(self):
        ...
        tasks = [
            ("sp_dim.sql", self.base_data_input_filename_path['sp_dim'] ), #"df_sp_dim.feather"
            ("sp_location_map.sql", self.base_data_input_filename_path['sp_location_mapping'] ), #"df_sp_location_mapping.feather"),
            ("get_customer_dim.sql", self.base_data_input_filename_path['customer_dim'] ), #"df_customer_dim.feather"),
            ("sp_active_customers.sql", self.base_data_input_filename_path['sp_active_customers'] ), #"df_sp_active_customers.feather"),
        ]
    ...
    def fetch_all_parallel(self, max_workers=None):
        ...
        # Prepare tasks 
        tasks = [
            ("sp_dim.sql", self.base_data_input_filename_path['sp_dim']),
            ("sp_location_map.sql", self.base_data_input_filename_path['sp_location_mapping']),
            ("get_customer_dim.sql", self.base_data_input_filename_path['customer_dim']),
            ("sp_active_customers.sql", self.base_data_input_filename_path['sp_active_customers']),
        ]
    ...
    def fetch_all_data_via_sp(self):
        """
        Fetch data from stored procedure usp_GetBeatAndRouteInputData and save as parquet files.
        ...
        file_mappings = {
            'spLocationMapping': self.base_data_input_filename_path['sp_location_mapping'],
            'spDim': self.base_data_input_filename_path['sp_dim'], 
            'ActiveCustomers': self.base_data_input_filename_path['sp_active_customers'],
            'CustomerDim': self.base_data_input_filename_path['customer_dim'],
            'AgentCustomer': self.base_data_input_filename_path['agent_customer']
        }
        
        # Table names in order they are returned by the stored procedure
        table_names = ['spLocationMapping', 'spDim', 'ActiveCustomers', 'CustomerDim', 'AgentCustomer']
        ...
        try:
            ...
            connection = pyodbc.connect(self.repl_con_string)
            cursor = connection.cursor()
            
            # Execute stored procedure
            self.logger.info("Executing stored procedure: usp_GetBeatAndRouteInputData")
            cursor.execute("EXEC usp_GetBeatAndRouteInputData")
            
            # Process each result set
            for i, table_name in enumerate(table_names):
                self.logger.info(f"Processing result set {i+1}: {table_name}")
                ...
                if rows:
                    ...
                    file_path = self.input_dir / file_mappings[table_name]
                    ...
                    result_summary[table_name] = len(df)
                ...
                # Move to next result set (if not the last one)
                if i < len(table_names) - 1:
                    if cursor.nextset():
                        self.logger.debug(f"Moved to next result set")
                    else:
                        self.logger.warning(f"Expected more result sets, but nextset() returned False")
                        break
            
            self.logger.info("Successfully processed all result sets from stored procedure")
    ...
    # Alternative implementation with more robust error handling and validation
    def fetch_all_data_via_sp_robust(self):
        """
        Robust version with additional error handling and data validation.
        """
        if not self.repl_con_string:
            self._get_connection_string()
        ...
        with pyodbc.connect(self.repl_con_string) as connection:
            cursor = connection.cursor()
                ...
                for i, table_name in enumerate(table_names):
                    try:
                        ...
                        if cursor.description is None:
                            self.logger.warning(f"No result set available for {table_name}")
                            result_summary[table_name] = 0
                            continue
                        ...
                        if i < len(table_names) - 1:
                            if not cursor.nextset():
                                self.logger.warning(f"Expected {len(table_names)} result sets, but only got {i+1}")
                                break
    ...
    def run_data_fetch(self):
        ...
        success = self.fetch_all_data_via_sp()
        
        if success:
            self.logger.info("All datasets fetched successfully")
        else:
