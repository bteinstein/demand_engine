...
def display_selection_summary(selected_spids, processed_sp_dim_df, 
    ...
    with col1:
        ...
        st.metric("Active", f"{int(active_count):,}")
    ...
    with col2:
        ...
        if sps_summaries is not None and not sps_summaries.empty:
            ...
            st.metric("Avg. Cust. per Beat", f"{int(avg_customers_per_beat):,}")
            ...
            st.metric("Avg. Active Cust. per Beat", f"{int(avg_active_customers_per_beat):,}")
            
            avg_beats_per_oam = selected_summaries['beats_per_oam'].mean() if not selected_summaries.empty else 0
            st.metric("Avg. Beat per Agent", f"{int(avg_beats_per_oam):,}")
        else:
            st.metric("Avg. Cust. per Beat", "0")
            st.metric("Avg. Active Cust. Per Beat", "0")
            st.metric("Avg. Beat per Agent", "0") 
        ...
        st.metric("Number of Agent Per MFC", f"{int(n_agents):,}")
...
def main():
    ...
    try:
        ...
        st.error(f"Error computing summaries data: {str(e)}")
        return
    ...
    if not selected_spids:
        ...
        col1, col2 = st.columns([8, 2])  # Map takes 90%, Summary takes 10% 
            ...
            with st.spinner("🗺️ Generating map..."):
                try:
                    ...
                    map_data = st_folium.st_folium(
                        folium_map, 
                        use_container_width=True, # Use percentage  
                        height=600 
                    )
        ...
        with st.expander("📊 Detailed Data"):
            tab1, tab2, tab3, tab4 = st.tabs(["Stockpoints", "Beats", "Customers", "Agents"])
            ...
            with tab4:
                ...
                if not mfc_agents.empty:
                    ...
                    st.info("No agent data available for selected stockpoints")
