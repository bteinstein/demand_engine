...
import numpy as np
import colorsys  
import branca.element # Import branca.element for IFrame
...
class ClusterRouteMapper:
    ...
    def _prepare_color_and_cluster_data(self):
        ...
        for cluster_id in self._unique_clusters:
            ...
            cluster_name = f"Cluster {cluster_id} - {lga_str} - {lcda_str} ({len(cluster_data)} Outlets)"
    ...
    def _create_popup_html(self, row, cluster_id):
        """
        Create HTML content for marker popups, ensuring no duplicate LGA/LCDA
        and returning a branca.element.IFrame.
        """
        ...
        # Use a set to track columns already added to avoid duplicates
        added_keys = set()

        # Define a preferred order for common columns, if they exist in popup_cols
        # This ensures a consistent display order for key information
        ordered_display_cols = []
        if 'CustomerID' in self.popup_cols: ordered_display_cols.append('CustomerID')
        if 'CustomerName' in self.popup_cols: ordered_display_cols.append('CustomerName')
        if 'Revenue' in self.popup_cols: ordered_display_cols.append('Revenue')
        if 'TotalQuantity' in self.popup_cols: ordered_display_cols.append('TotalQuantity')

        # Add columns from ordered_display_cols first
        for col in ordered_display_cols:
            if col in row.index and pd.notna(row[col]):
                display_name = ' '.join(word.capitalize() for word in col.split('_'))
                popup_html += f"<div style='margin: 2px 0;'><strong>{display_name}:</strong> {row[col]}</div>"
                added_keys.add(col)

        # Add LGA and LCDA, ensuring no duplication and proper display name
        if self.lga_col in row.index and pd.notna(row[self.lga_col]) and self.lga_col not in added_keys:
            popup_html += f"<div style='margin: 2px 0;'><strong>LGA:</strong> {row[self.lga_col]}</div>"
            added_keys.add(self.lga_col)
        if self.lcda_col in row.index and pd.notna(row[self.lcda_col]) and self.lcda_col not in added_keys:
            popup_html += f"<div style='margin: 2px 0;'><strong>LCDA:</strong> {row[self.lcda_col]}</div>"
            added_keys.add(self.lcda_col)

        # Add any remaining popup_cols that haven't been added yet
        for col in self.popup_cols:
            if col not in added_keys and col in row.index and pd.notna(row[col]):
                display_name = ' '.join(word.capitalize() for word in col.split('_'))
                popup_html += f"<div style='margin: 2px 0;'><strong>{display_name}:</strong> {row[col]}</div>"
                added_keys.add(col)

        # Add Coordinates (always last, and ensure it's not duplicated if it was somehow in popup_cols)
        coord_key = 'Coordinates'
        if coord_key not in added_keys:
            popup_html += (f"<div style='margin: 2px 0;'><strong>{coord_key}:</strong> "
                           f"{row[self.lat_col]:.4f}, {row[self.lon_col]:.4f}</div>")
            added_keys.add(coord_key)

        popup_html += "</div>"
        
        # Wrap the HTML string in a branca.element.IFrame for proper rendering
        # Adjust width and height as needed for your content
    ...
    def _add_markers_to_map(self, df_chunk):
        ...
        for idx, row in df_chunk.iterrows():
            ...
            # Call _create_popup_html which now returns a branca.element.IFrame
            popup_iframe = self._create_popup_html(row, cluster_id)
            tooltip_content = self._create_tooltip_content(row, cluster_id)

            marker = folium.CircleMarker(
                location=[row[self.lat_col], row[self.lon_col]],
                # Pass the IFrame directly to folium.Popup
            ...
            if current_trip_destinations:
                for j, dest_data in enumerate(current_trip_destinations):
                    ...
                    if row_data is not None:
                        # Use the rich popup from the cluster data, which now returns an IFrame
                        cluster_id = row_data[self.cluster_col]
                        popup_iframe = self._create_popup_html(row_data, cluster_id)
                        ...
                        fallback_html = (f"<b>Trip {trip_id} - Destination {j+1}</b><br>"
                                         f"Customer ID: {customer_id}<br>"
                                         f"Coordinates: {dest_latlon[0]:.4f}, {dest_latlon[1]:.4f}")
                        # Create an IFrame for the fallback HTML as well
                        popup_iframe = branca.element.IFrame(html=fallback_html, width=300, height=100)
        ...
        for i, (cluster_id, stats) in enumerate(sorted_cluster_items[:cluster_count]):
            ...
            <div style="margin: 3px 0; display: flex; align-items: center; padding: 2px;">
                ...
                <span style="font-size: 10px; flex: 1;">
                    ...
                    <br><span style="color: #666; font-size: 9px;">{stats['count']} Outlets</span>
                </span>
            </div>
            '''
    ...
    def _create_cluster_control_js(self, n_clusters, total_points):
        ...
        setTimeout(function() {{
            ...
            summaryDiv.innerHTML = 'Total: {n_clusters} clusters | {total_points} Outlets';
...
if __name__ == "__main__":
    ...
    data = {
        ...
        'TotalQuantity': [10, 15, 12, 18, 20, 9, 16, 22, 11, 17],
        'CustomerID': [f'CUST{1000 + i}' for i in range(10)]
    }
        ...
        "Trips": [
            {
                "TripID": "TRIP001",
                "Destinations": [
                    {"CustomerID": "CUST1000", "Coordinate": [3.35, 6.45]}, # Lon, Lat
                    {"CustomerID": "CUST1002", "Coordinate": [3.3, 6.6]}
                ]
            },
            {
                "TripID": "TRIP002",
                "Destinations": [
                    {"CustomerID": "CUST1001", "Coordinate": [3.4, 6.5]},
                    {"CustomerID": "CUST1003", "Coordinate": [3.38, 6.48]}
                ]
            }
        ]
    }
    ...
    mapper = ClusterRouteMapper(
        ...
        lga_col='LGA',
        lcda_col='LCDA',
        popup_cols=['CustomerName', 'Revenue', 'TotalQuantity', 'CustomerID'], # Added CustomerID and TotalQuantity
