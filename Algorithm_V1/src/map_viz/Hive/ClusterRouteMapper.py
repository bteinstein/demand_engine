...
class ClusterRouteMapper:
    ...
    def _create_popup_html(self, row, cluster_id):
        """Create HTML content for marker popups using branca.element.Html."""
        popup_html = f"""
        <div style="font-family: Arial, sans-serif; font-size: 12px; padding: 10px;">
            <div style="font-weight: bold; color: {self._cluster_color_map.get(cluster_id, '#000000')}; margin-bottom: 5px;">
                Cluster {cluster_id}
            </div>
            {''.join(f'<div style="margin: 2px 0;"><strong>{col}:</strong> {row[col]}</div>'
                     for col in self.popup_cols if col in row.index and pd.notna(row[col]))}
            <div style="margin: 2px 0;"><strong>Coordinates:</strong> {row[self.lat_col]:.4f}, {row[self.lon_col]:.4f}</div>
            {f'<div style="margin: 2px 0;"><strong>LGA:</strong> {row[self.lga_col]}</div>'
             if self.lga_col in row.index and pd.notna(row[self.lga_col]) else ''}
            {f'<div style="margin: 2px 0;"><strong>LCDA:</strong> {row[self.lcda_col]}</div>'
             if self.lcda_col in row.index and pd.notna(row[self.lcda_col]) else ''}
        </div>
        """
        return branca.element.Html(popup_html, script=False)
    ...
    def _add_routes_to_map(self):
        ...
        for i, route_info in enumerate(self.routes_info):
            ...
            route_color = self._get_folium_color(trip_id, type='route')
            ...
            if current_trip_destinations:
                for j, dest_data in enumerate(current_trip_destinations):
                    ...
                    if row_data is not None:
                        ...
                        marker_color = self._get_folium_color(cluster_id, type='cluster')
                    else:
                        # Fallback if coordinate is not in the original DataFrame
                        customer_id = dest_data.get('CustomerID', 'N/A')
                        popup_html = branca.element.Html(
                            f"""
                            <div style="font-family: Arial, sans-serif; font-size: 12px; padding: 10px;">
                                <b>Trip {trip_id} - Destination {j+1}</b><br>Customer ID: {customer_id}
                            </div>
                            """, script=False
                        )
                        tooltip_content = f"Dest {j+1} for Trip {trip_id} ({customer_id})"
                        marker_color = self._get_folium_color(trip_id, type='route')
    ...
    def _update_legend_html(self):
        ...
        for i, route_info in enumerate(valid_routes[:route_count]):
            route_color = self._route_color_map.get(route_info['TripID'], '#808080')
            trip_id = route_info['TripID']
            outlet_count = route_info['outlet_count']
            legend_html += f'''
            <div style="margin: 3px 0; display: flex; align-items: center; padding: 2px;">
