...
import numpy as np
import colorsys
import branca # Import branca for IFrame
...
class ClusterRouteMapper:
    ...
    def _create_popup_html(self, row, cluster_id):
        """
        Create HTML content for marker popups and wrap it in a branca.element.IFrame.
        Returns a branca.element.IFrame object.
        """
        popup_html_content = "<div style='font-family: Arial, sans-serif; font-size: 12px;'>"
        popup_html_content += (f"<div style='font-weight: bold; color: {self._cluster_color_map.get(cluster_id, '#000000')};"
                       "margin-bottom: 5px;'>")
        popup_html_content += f"Cluster {cluster_id}</div>"

        # Keep track of columns already added to avoid duplication
        added_cols = set()

        for col in self.popup_cols:
            if col in row.index and pd.notna(row[col]):
                popup_html_content += f"<div style='margin: 2px 0;'><strong>{col}:</strong> {row[col]}</div>"
                added_cols.add(col)

        popup_html_content += (f"<div style='margin: 2px 0;'><strong>Coordinates:</strong> "
                       f"{row[self.lat_col]:.4f}, {row[self.lon_col]:.4f}</div>")

        # Add LGA and LCDA only if they were not already added via popup_cols
        if self.lga_col in row.index and pd.notna(row[self.lga_col]) and self.lga_col not in added_cols:
            popup_html_content += f"<div style='margin: 2px 0;'><strong>LGA:</strong> {row[self.lga_col]}</div>"
        if self.lcda_col in row.index and pd.notna(row[self.lcda_col]) and self.lcda_col not in added_cols:
            popup_html_content += f"<div style='margin: 2px 0;'><strong>LCDA:</strong> {row[self.lcda_col]}</div>"

        popup_html_content += "</div>"

        # Wrap the HTML content in a branca IFrame for proper rendering
        # Adjust width and height as needed to prevent scrollbars for typical content
        iframe = branca.element.IFrame(html=popup_html_content, width=200, height=150) # Increased height slightly
        return iframe
    ...
    def _add_markers_to_map(self, df_chunk):
        ...
        for idx, row in df_chunk.iterrows():
            ...
            # _create_popup_html now returns an IFrame object
            ...
            marker = folium.CircleMarker(
                ...
                # Pass the IFrame object directly to folium.Popup
                popup=folium.Popup(popup_iframe), # Removed parse_html=True
    ...
    def _add_routes_to_map(self):
        ...
        for i, route_info in enumerate(self.routes_info):
            ...
            route_color = self._get_folium_color(trip_id, type='route') # Already a Folium color name
            ...
            if current_trip_destinations:
                for j, dest_data in enumerate(current_trip_destinations):
                    ...
                    if row_data is not None:
                        ...
                        popup_iframe = self._create_popup_html(row_data, row_data[self.cluster_col])
                        tooltip_content = self._create_tooltip_content(row_data, row_data[self.cluster_col])
                        marker_color = self._get_folium_color(row_data[self.cluster_col], type='cluster')  # Folium-compatible cluster color
                    else:
                        # Fallback if coordinate is not in the original DataFrame
                        customer_id = dest_data.get('CustomerID', 'N/A')
                        # Create a simple HTML string and wrap it in an IFrame
                        fallback_html = f"<b>Trip {trip_id} - Destination {j+1}</b><br>Customer ID: {customer_id}"
                        popup_iframe = branca.element.IFrame(html=fallback_html, width=200, height=100)
                    ...
                    folium.Marker(
                        ...
                        popup=folium.Popup(popup_iframe), # Pass the IFrame object
                        tooltip=tooltip_content,
