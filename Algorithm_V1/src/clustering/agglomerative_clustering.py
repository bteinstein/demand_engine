...
class AgglomerativeGeographicClustering(BaseGeographicClustering):
    def __init__(self, max_customers_per_cluster=20, max_distance_km=50, 
                 linkage_method='ward', sub_cluster_if_oversized=True, logger=logging.getLogger(__name__)):
        """
    ...
    def agglomerative_clustering(self, customers_df):
        ...
        self.logger.info(f"Calculating {n_customers*(n_customers-1)//2} pairwise distances...")
        ...
        if n_customers > 2000 and self.linkage_method != 'ward': # Ward only works with Euclidean-like pdist
            ...
            self.logger.info("Warning: Large dataset for pdist. This might take a while or consume a lot of memory.")

        distances = self.haversine_pdist(coords_array)
        
        # Step 2: Perform hierarchical clustering using linkage
        self.logger.info(f"Performing linkage using '{self.linkage_method}' method...")
        ...
        self.logger.info(f"Cutting dendrogram at max_distance_km={self.max_distance_km}...")
        ...
        self.logger.info(f"Post-processing clusters for size constraint (max {self.max_customers_per_cluster} customers)...")
        for current_cluster_label in sorted(customers_df['cluster_temp'].unique()):
            ...
            if len(cluster_indices) > self.max_customers_per_cluster and self.sub_cluster_if_oversized:
                self.logger.info(f"  Cluster {current_cluster_label} (size {len(cluster_indices)}) is oversized. Sub-clustering...")
        ...
        self.logger.info("Agglomerative clustering completed.\n")
        return result_df
