...
import logging
# Suppress warnings (useful for specific scenarios, but use with caution)
...
class OptimizedDivisiveGeographicClustering:
    def __init__(self, max_customers_per_cluster=20, max_distance_km=50, 
                 use_vectorized_distances=True, balance_clusters=False, logger=logging.getLogger(__name__)):
...
def compare_performance():
    ...
    self.logger.info("Testing Optimized Version...")
    ...
    self.logger.info(f"Optimized clustering completed in {optimized_time:.2f} seconds")
    self.logger.info(f"Number of clusters: {optimized_result['cluster'].nunique()}")
    self.logger.info(f"Customers per cluster (avg): {len(optimized_result) / optimized_result['cluster'].nunique():.1f}")
