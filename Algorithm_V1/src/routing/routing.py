...
def get_valhalla_routes_info(trip_data: dict, client = None, logger = logging.getLogger(__name__)) -> list:
    """
    ...
    if not client:
        # If no client is provided, create a new Valhalla client instance
        try:
            print("Setting up Valhalla client...")
            client = Valhalla(base_url=VALHALLA_BASE_URL) 
            print("Valhalla client setup complete.")
        except Exception as e:
            print(f"Error setting up Valhalla client: {e}")
            return []

    routes_info = []
    logger.info("Starting route calculations for all trips...")
    ...
    for trip in trips:
        ...
        try:
            logger.debug(f"  Calculating route for Trip ID: {trip_id} ({len(all_waypoints_for_trip)} waypoints)...")
            ...
            logger.debug(f"  Trip ID {trip_id} calculated. Distance: {route_response.distance / 1000:.2f} km, Duration: {route_response.duration / 60:.2f} min")

        except Exception as e:
            logger.info(f"  Error calculating route for Trip ID {trip_id}: {e}")
            routes_info.append({
                'TripID': trip_id,
                'error': str(e)
            })
    logger.info("All route calculations completed.\n")
    return routes_info
...
def plot_routes_on_map(trip_data: dict, routes_info: list, output_filename: str = "valhalla_trips_map.html"):
    ...
    m = folium.Map(location=[stock_point_coord[1], stock_point_coord[0]], zoom_start=11, tiles='cartodb positron')
